...
const Members = () => {
  ...
  const handleSaveMember = async (memberData) => {
    try {
      if (editingMember) {
        await updateMember({ id: editingMember._id, ...memberData }).unwrap();
      } else {
  ...
  const handleToggleStatus = async (member) => {
    try {
      await updateMember({ id: member._id, isActive: !member.isActive }).unwrap();
    } catch (err) {
      ...
      {viewingMember && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full">
            <h3 className="text-xl font-bold mb-4">{viewingMember.name}</h3>
            <p>Email: {viewingMember.email}</p>
            <p>Phone: {viewingMember.phone}</p>
            <p>Membership Type: {viewingMember.membershipType}</p>
            <p>
              Status: {viewingMember.isActive ? "Active" : "Inactive"}
            </p>
            <button
